...
ensemble learning with right-censored data. JSSL jointly
learns prediction models for the event-time and censoring
distributions. JSSL is based on the simple idea of an
...
guarantees. Section~\ref{sec:numer-exper} reports the results of
...
the library, without any knowledge of the data-generating distribution.
...
JSSL immediately provides estimates of these
...
\begin{proof}[Proof of Corollary~\ref{cor:asymp-cons}]
  ...
  Proposition~\ref{prop:oracle-prop} by letting $\delta$ grow to zero
  with \( n \), for instance as $\delta_n = \log(n)^{-\epsilon}$ for
  some $\epsilon>0$.
\end{proof}
