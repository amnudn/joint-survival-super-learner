...
\fxnote*{todo?}{[Should maybe define the oracle here]}

We shall compare the super learner theoretically to the so-called oracle
learner, which is defined as the learner that minimises the expected loss
under the data-generating distribution \( P \), i.e.,
\begin{equation*}
  \tilde{a}_n =
  ...
  ,
\end{equation*}
where we use \( \E_P \) to denote expectation under the distribution
\( P \). Note that both the discrete super learner and the oracle
learner depend on the library of learners and on the specific
partition of the data into the folds
\( \data_n^1, \dots, \data_n^K \). In addition, the oracle learner is
a function of the unknown data-generating distribution.
...
\begin{corollary}
  ...
  \item If $\alpha=1$ then
    \(\frac{1}{K}\sum_{k=1}^{K} \E_{P}{\left[ \Vert \hat{\phi}_n(\data_n^{-k}) - F_P \Vert_{P}^2
...
[Explain in terms of types of learners: Parametric (case (a)) and ML
(case (b))]
