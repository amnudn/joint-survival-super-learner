...
pre-specified estimator of the censoring distribution. We establish
theoretical guarantees, including a finite-sample oracle inequality
and a bound on the average price for using cross-validation. Through
empirical studies we show that our super learner is able to better
account for different censoring mechanisms than existing methods. We
...
\begin{equation}\label{eq:discrete-JSLL}
  \hat{\phi}_n = (\hat \Lambda_{1n},\hat \Lambda_{2n}, \hat \Gamma_{n})
...
Proposition~\ref{prop:stric-prop} states that the integrated Brier
score as defined in Section~\ref{sec:joint-survival-super-learner} is
a strictly proper scoring rule. To establish this result, recall that
the function \(F\) implicitly depends on the data-generating
probability measure \(P\in\mathcal P\) but that this was so-far
suppressed in the notation. We now make this dependence explicit by
writing \(F_P\) for the function determined by a given \(P
\in\mathcal{P}\) in accordance with equation
...
benchmark it against the data-generating distribution \( F_P \), which
according to Proposition~\ref{prop:stric-prop} has the smallest
expected loss. Another useful benchmark for the joint survival super
learner is the best learner included in the library of learners. The
so-called oracle learner is formally defined as
...
we use \( \E_P \) to denote the expectation under the distribution \( P \)
for a new observation \( O \) which is independent of the data \( \data_n^{-k} \). Like
...
Proposition~\ref{prop:oracle-prop} provides a finite-sample bound on
the average price we pay for using cross-validation, and
