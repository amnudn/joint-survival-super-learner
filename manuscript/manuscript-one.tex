...
data and illustrate its performance in small samples with simulated
data.
\end{abstract}
...
\begin{equation*}
  ...
  \left(
      F(t,l,X) - \1{\{\eta(t)=l\}}
  \right)^2.
\end{equation*} The Brier score is the average squared prediction
error across the four states. Based on a split of a data set
\(\data_n\) into $K$ disjoint approximately equally sized subsets
...
% ANDERS: to me Poisson regression is a way to fit a Cox model
...
near-parametric rate of convergence. % ANDERS: what does "taken equal to mean"? 
...
as Cox lasso or random survival forests.%ANDERS: need references here
...
learner achieves the same convergence rate as the best-performing % ANDERS: best performing with respect to our integrated Brier score on F?
...
estimate the conditional survival function. In order to enable a
comparison with existing super learner methods we restrict the
attention to a setting without competing risks. We report the results
of the numerical experiments in two parts. In the first part, we
compare the joint survival super learner with two IPCW based discrete
...
For our numerical experiments we have synthesized the data of the
prostate cancer study of \cite{kattan2000pretreatment} based on
structural equations where we specify Cox-Weibull regression models
\cite{Bender2005} for the latent event and censoring times. The
...
Section~\ref{sec:joint-survival-super-learner}, with the special case of
...
Section~\ref{sec:joint-survival-super-learner}) for all learners are shown in
