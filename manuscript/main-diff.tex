...
%DIF DEL main-first-submit-version.tex   Fri Nov 14 10:47:47 2025
%DIF ADD main.tex                        Fri Nov 14 15:38:47 2025
...
%DIF -------
\usepackage{amsfonts,siunitx} %DIF > 
%DIF -------

\usepackage[colorlinks,allcolors=black]{hyperref}
%DIF 45a45-46
\usepackage[ruled]{algorithm2e} %DIF > 
\SetArgSty{textnormal} %DIF > 
%DIF -------

% Notation
%DIF 47c49
%DIF < \usepackage{dsfont,enumitem, url}
%DIF -------
\usepackage{enumitem, url} %DIF > 
%DIF -------
...
mispecified model. In survival analysis, a super learner may for
example combine a stack of Cox regression models with a stack of
random survival forests \citep[][Section 8.4]{gerds2021medical}. Such
a strategy has recently produced \textit{KDpredict}
...
the learners, the super learner behind \DIFdelbegin \DIFdel{KDpredict }\DIFdelend \DIFaddbegin \textit{\DIFadd{KDpredict}} \DIFaddend uses inverse
...
show that the discrete joint survival super learner is consistent when
the library of learners includes at least one consistent learner. We
also derive a finite-sample oracle inequality for the discrete joint
survival super learner. We demonstrate how to construct a library of
learners using common methods for survival analysis and illustrate the
use of the joint survival super learner using \DIFaddbegin \DIFadd{data from }\DIFaddend a prostate
cancer \DIFdelbegin \DIFdel{data set}\DIFdelend \DIFaddbegin \DIFadd{study \mbox{%DIFAUXCMD
\cite{kattan2000pretreatment} }\hskip0pt%DIFAUXCMD
and also synthesize the
prostate cancer data to generate a realistic scenario for our
simulation experiments}\DIFaddend .
...
are not observed in the training set. \DIFaddbegin \DIFadd{For instance, if no competing
...
all probability mass is assigned to the finite number of time points
at which the cumulative hazard function jumps. The likelihood
according to such a model is zero at almost all time points, and thus
the likelihood of any hold-out sample will almost surely be zero when
data are observed in continuous time. }\DIFaddend This problem occurs with
...
learner proposed by \cite{polley2011-sl-cens}. \DIFaddbegin \DIFadd{A reviewer suggested
that one might approach this issue by post-hoc smoothing the piecewise
constant cumulative hazard functions. This is an interesting idea which we
however do not pursue further in this work.
}

\DIFaddend When a proportional hazards model is assumed, the baseline hazard
...
predict the observed data\DIFaddbegin \DIFadd{, }\DIFaddend and the discrete joint survival super
...
\begin{equation}\label{eq:discrete-JSLL}
  ...
  \hat{R}_{n}(\phi_{a_1,a_2,b} ; \bar{B}_{\tau}).
\end{equation}
\DIFaddbegin \DIFadd{We provide a summary of the procedure for obtaining the joint survival
...
estimator of the censoring \DIFdelbegin \DIFdel{distribution:
}\begin{displaymath}
 \DIFdel{\hat G_n(t \mid X=x) = \exp\left\{-\hat\Gamma_n(t \mid x)\right\}.
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{survival function:
}\begin{equation}\DIFadd{\label{eq:estimate-G}
 \hat G_n(t \mid X=x) = \exp\left\{-\hat\Gamma_n(t \mid x)\right\}.
}\end{equation}\DIFaddend 

\DIFaddbegin \begin{algorithm}[H]
  \label{algo:jossl}
  \caption{\DIFadd{The joint survival super learner using the integrated Brier
    score}}
  ...
  \KwOut{Selected tuple of learner \( (\hat{\Lambda}_{1n},
    ...
    \mathcal{A}_2 \times \mathcal{B} \), estimated risk prediction
    model \(\hat Q_n\), and estimated censoring distribution \( \hat G_n \).
  }

  \DIFadd{Partition the data set \( \data_n \) randomly into \( K \) subsets
  ...
  \For{$k = 1, \dots, K$}{
    ...
    \item[\DIFadd{\( \hat{\mathcal{A}}_1^{-k} \)}]         
      \(  = \{ a_1(\data_n^{-k}) : a_1 \in
        \mathcal{A}_1 \}  \),
      \item[\DIFadd{\( \hat{\mathcal{A}}_2^{-k} \)}]         
        \(  = \{ a_2(\data_n^{-k}) : a_2 \in
        \mathcal{A}_2 \}  \),
      \item[\DIFadd{\( \hat{\mathcal{B}}^{-k} \)}]         
        \(  = \{ b(\data_n^{-k}) : b \in
        \mathcal{B} \}  \) ,
    \end{list}
    ...
    \Return{\( (\hat{\Lambda}_{1n},
      ...
      = \argmin_{(a_1,a_2,b)\in \mathcal{A}_1\times\mathcal{A}_2\times\mathcal{B}}
    \hat{R}_{n}(\phi_{a_1,a_2,b} ; \bar{B}_{\tau}) \) and \(\hat Q_n, \hat G_n\) using on equations \eqref{eq:cs-risk-def}, \eqref{eq:estimate-G}.}
\end{algorithm}


\DIFaddend \section{Theoretical guarantees}
...
The expectation in Proposition~\ref{prop:oracle-prop} is taken with
respect to the product measure \( P^{n} \) for the data set
...
true error rate \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{by }\DIFaddend \cite{efron_and_tibshirani97}. As with many
...
data-dependent, unknown quantities. \DIFdelbegin \DIFdel{However, it does quantify how the
number of folds, the time horizon, and the number of learners in the
library can be expected to influence the performance. }\DIFdelend The result has the following
...
hazard functions \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{hjort1992inference}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{hjort1992inference}}\hskip0pt%DIFAUXCMD
}\DIFaddend , and hence
...
\DIFaddbegin \DIFadd{The norm defined in equation~(\ref{eq:norm}) operates on functions \(
...
(\ref{eq:transition}). Based on this correspondence we postulate that
...
\DIFaddend \section{Numerical experiments}
...
structural equation model under parametric assumptions. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{As }\DIFaddend outcome of
interest \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{we choose }\DIFaddend the time from randomization until the combined
endpoint tumour recurrence or all-cause death. \DIFaddbegin \DIFadd{This is a scenario
without competing risks. }\DIFaddend Five baseline covariates are used to predict
the outcome risk: prostate-specific antigen (PSA, ng/mL), Gleason
score sum (GSS, values between 6 and 10), radiation dose (RD), hormone
therapy (HT, yes/no) and clinical stage (CS, six values). The study
was designed such that a patient's radiation dose depended on when the
patient entered the study. This in turn implies that the time of
censoring depends on the radiation dose. The data were re-analysed in
\citep{gerds2013estimating} where a sensitivity analysis was conducted
based on simulated data. Here we use the same simulation setup, where
event and censoring times are generated according to parametric
Cox-Weibull models \citep{Bender2005} estimated from the original
...
\DIFdelbegin \DIFdel{function}\DIFdelend \DIFaddbegin \DIFadd{functions}\DIFaddend , we obtain a \DIFdelbegin \DIFdel{risk prediction model as described in
Section~\ref{sec:joint-survival-super-learner},
with the special case
of }\DIFdelend \DIFaddbegin \DIFadd{predicted risk of the combined outcome event,
c.f., equation \eqref{eq:cs-risk-def}, where in the current scenario
without competing risks, }\DIFaddend $\Lambda_2 = 0$\DIFaddbegin \DIFadd{, and \(\hat Q_n(T \leq t, D =
1 \mid X=x)=1-\exp\{-\hat\Lambda_{1n}(t \mid x)\)}\DIFaddend . We measure the
performance of \DIFdelbegin \DIFdel{each super learner
}\DIFdelend \DIFaddbegin \DIFadd{the risk prediction models }\DIFaddend by calculating the index of
prediction accuracy (IPA) \citep{kattan2018index} at a fixed time
horizon (36 months)\DIFdelbegin \DIFdel{for the
}\DIFdelend \DIFaddbegin \DIFadd{. For a }\DIFaddend risk prediction model \DIFdelbegin \DIFdel{provided by the super learner. The IPA is 1
minus the ratio between the model's Brier score and the null model's
Brier score, where the null model is the model that }\DIFdelend \DIFaddbegin \DIFadd{\( r \colon
\mathcal{X} \rightarrow [0,1] \), IPA at time \( \tau \) is defined as
}\begin{equation*}
  \DIFadd{1 - \frac{\E_Q{[(r(X) - \1{\{T \leq \tau\}})^2]}}{\E_Q{[( Q{({T \leq \tau})} - \1{\{T \leq \tau\}})^2]}}.
  %DIF >  {\E_Q{[( \E_Q{[\1{\{T \leq \tau\}}]} - \1{\{T \leq \tau\}})^2]}}
}\end{equation*}
\DIFadd{We chose IPA as a performance measure because it is proper,
...
null model which }\DIFaddend does not use any covariate information. The \DIFdelbegin \DIFdel{value of IPA is approximated using a large (\( n = 20,000 \)) }\DIFdelend \DIFaddbegin \DIFadd{definition
...
IPA in a large }\DIFaddend independent data set of
uncensored \DIFdelbegin \DIFdel{data}\DIFdelend \DIFaddbegin \DIFadd{survival times (\( n = 20,000 \))}\DIFaddend .
As a benchmark, we calculate the performance of the risk
prediction model chosen by the oracle selector, which has the highest
IPA in the simulation setting.
%DIF >  ANDERS: this sounds as if the oracle selector is defined to be the
%DIF >          super learner that has the highest IPA ...
...
\begin{figure}[ht]
  ...
  \caption{For the risk prediction models provided by each of the
    super learners, the IPA is plotted against sample size \DIFaddbeginFL \DIFaddFL{(on
    \( \log_2 \) scale)}\DIFaddendFL . The results are averages across 1000
    simulated data sets and the error bars are used to quantify the
    Monte Carlo uncertainty. JSSL denotes the joint survival super
    learner. }
\label{fig:ipcw-fail}
\end{figure}
...
model. \DIFdelbegin \DIFdel{Both methods }\DIFdelend \DIFaddbegin \DIFadd{The super learner }{\it \DIFadd{survSL}} \DIFadd{also implements an ensemble
learning approach, combining multiple learners to improve predictive
performance.  The major difference is that }{\it \DIFadd{survSL}} \DIFadd{iterates
between prediction of the outcome probabilities and prediction of the
censoring probabilities, where the joint survival super learner
evaluates all learners simultaneously on the level of the observed
data distribution. Another difference is that }{\it \DIFadd{survSL}} \DIFadd{creates a
meta-learner that combines the predictions of the individual learners,
whereas the joint survival super learner applied in the simulation
study is a discrete super learner. Since both approaches }\DIFaddend provide
estimates of the event-time and \DIFdelbegin \DIFdel{censoring
distributions and hence }\DIFdelend \DIFaddbegin \DIFadd{the censoring distributions, }\DIFaddend we
compare their performance with respect to both\DIFdelbegin \DIFdel{the outcome and the censoring distribution}\DIFdelend . Again we use the IPA
...
similar or higher IPA compared to \DIFdelbegin \DIFdel{survSL }\DIFdelend \DIFaddbegin {\it \DIFadd{survSL}} \DIFaddend with respect to both
...
whereas \DIFdelbegin \DIFdel{survSL }\DIFdelend \DIFaddbegin {\it \DIFadd{survSL}} \DIFaddend combines the learners. \DIFaddbegin \DIFadd{We conjecture that a
...
discrete joint survival super learner outperforms }{\it \DIFadd{survSL}} \DIFadd{in this
scenario.
}\DIFaddend 
...
\begin{figure}[ht]
  ...
  \caption{For the risk prediction models of the outcome provided by
    ...
    plotted against sample size \DIFaddbeginFL \DIFaddFL{(on \( \log_2 \) scale)}\DIFaddendFL . The results
    are averages across 1000 repetitions and the error bars are used
    to quantify the Monte Carlo uncertainty. JSSL denotes the joint
...
\begin{figure}[ht]
  ...
  \caption{For the risk prediction models of the censoring model
    ...
    horizon is plotted against sample size \DIFaddbeginFL \DIFaddFL{(on \( \log_2 \)
    scale)}\DIFaddendFL . The results are averages across 1000 repetitions and the
    error bars are used to quantify the Monte Carlo uncertainty. JSSL
...
\DIFaddbegin \DIFadd{The figure shows that the differences between the conventional method
...
\DIFadd{These results illustrate that the joint survival super learner
...
both. \DIFaddbegin \DIFadd{However, our numerical experiments show that our approach can
...
transformations, and pseudo-values, were discussed in Section
\ref{sec:super-learning}. As mentioned, the drawback of these
...
underlying the super learner }{\it \DIFadd{survSL}}
\DIFadd{\mbox{%DIFAUXCMD
\citep{westling2021inference}}\hskip0pt%DIFAUXCMD
. Though we found this approach to
...
\mbox{%DIFAUXCMD
\citep[e.g.,][]{hjort1992inference,whitney2019comment}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 

A relevant application of the joint survival super learner is within
the framework of targeted learning \citep{van2011targeted} \DIFdelbegin \DIFdel{, also known
as }\DIFdelend \DIFaddbegin \DIFadd{or }\DIFaddend debiased
machine learning \citep{chernozhukov2018double}, \DIFdelbegin \DIFdel{-- a
general
methodology that combines }\DIFdelend \DIFaddbegin \DIFadd{which are general
methodologies for combining }\DIFaddend flexible, data-adaptive estimation of
nuisance parameters with asymptotically valid inference for
...
learner, but it is \DIFdelbegin \DIFdel{of interest }\DIFdelend \DIFaddbegin \DIFadd{straightforward }\DIFaddend to extend the method to \DIFdelbegin \DIFdel{a proper
ensemble learner, where learners are combined}\DIFdelend \DIFaddbegin \DIFadd{more
complex ensemble learning}\DIFaddend , e.g., through
stacking \DIFdelbegin \DIFdel{. How }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{breiman1996stacked}}\hskip0pt%DIFAUXCMD
. However, there are multiple possible
ways to apply stacking to the the joint survival super leaner. One
option is to construct a single convex combination of the \( F \)-learners
...
\( \mathcal{A}_2 \), and \( \mathcal{B} \).  How such }\DIFaddend an ensemble
should be build \DIFdelbegin \DIFdel{for tuples of learners }\DIFdelend \DIFaddbegin \DIFadd{and implemented }\DIFaddend is an interesting topic for future
research.

\DIFaddbegin \DIFadd{The findings from our simulation study suggest that the joint super
...
\DIFdelbegin %DIFDELCMD < \begin{proof}[of Proposition~\ref{prop:stric-prop}]
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \begin{proof}[Proof of Proposition~\ref{prop:stric-prop}]
  \DIFaddend The result follows from Lemma~\ref{lemma:norm}.
\end{proof}
...
\DIFdelbegin %DIFDELCMD < \begin{proof}[of Proposition~\ref{prop:oracle-prop}]
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \begin{proof}[Proof of Proposition~\ref{prop:oracle-prop}]
  \DIFaddend Minimizing the loss \( \bar{B}_{\tau} \) is equivalent to
...
\DIFdelbegin %DIFDELCMD < \begin{proof}[of Corollary~\ref{cor:asymp-cons}]
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \begin{proof}[Proof of Corollary~\ref{cor:asymp-cons}]
  \DIFaddend By definition of the oracle and Lemma~\ref{lemma:norm},
  \begin{equation*}
