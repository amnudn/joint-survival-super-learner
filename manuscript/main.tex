...
the learners, the super learner behind \textit{KDpredict} uses inverse
...
use of the joint survival super learner using data from a prostate
cancer study \cite{kattan2000pretreatment} and also synthesize the
...
data are observed in continuous time. This problem occurs with
...
learner proposed by \cite{polley2011-sl-cens}. A reviewer suggested
...
estimator of the censoring survival function:
\begin{equation}\label{eq:estimate-G}
...
true error rate by \cite{efron_and_tibshirani97}. As with many
...
data-dependent, unknown quantities. The result has the following
...
hazard functions \citep{hjort1992inference}, and hence
...
structural equation model under parametric assumptions. As outcome of
interest we choose the time from randomization until the combined
endpoint tumour recurrence or all-cause death. This is a scenario
without competing risks. Five baseline covariates are used to predict
...
functions, we obtain a predicted risk of the combined outcome event,
c.f., equation \eqref{eq:cs-risk-def}, where in the current scenario
without competing risks, $\Lambda_2 = 0$, and \(\hat Q_n(T \leq t, D =
1 \mid X=x)=1-\exp\{-\hat\Lambda_{1n}(t \mid x)\). We measure the
performance of the risk prediction models by calculating the index of
prediction accuracy (IPA) \citep{kattan2018index} at a fixed time
horizon (36 months). For a risk prediction model \( r \colon
...
% ANDERS: this sounds as if the oracle selector is defined to be the
%         super learner that has the highest IPA ...
...
\begin{figure}[ht]
  ...
  \caption{For the risk prediction models provided by each of the
    super learners, the IPA is plotted against sample size (on
    \( \log_2 \) scale). The results are averages across 1000
...
performance.  The major difference is that {\it survSL} iterates
...
data distribution. Another difference is that {\it survSL} creates a
...
study is a discrete super learner. Since both approaches provide
estimates of the event-time and the censoring distributions, we
compare their performance with respect to both. Again we use the IPA
...
whereas {\it survSL} combines the learners. We conjecture that a
...
\begin{figure}[ht]
  ...
  \caption{For the risk prediction models of the outcome provided by
    ...
    plotted against sample size (on \( \log_2 \) scale). The results
...
\begin{figure}[ht]
  ...
  \caption{For the risk prediction models of the censoring model
    ...
    horizon is plotted against sample size (on \( \log_2 \)
    scale). The results are averages across 1000 repetitions and the
...
The figure shows that the differences between the conventional method
...
the framework of targeted learning \citep{van2011targeted} or debiased
machine learning \citep{chernozhukov2018double}, which are general
methodologies for combining flexible, data-adaptive estimation of
...
learner, but it is straightforward to extend the method to more
complex ensemble learning, e.g., through
stacking \citep{breiman1996stacked}. However, there are multiple possible
