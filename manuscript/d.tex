...
%DIF DEL a.tex   Fri Jul  4 07:34:56 2025
%DIF ADD b.tex   Fri Jul  4 07:35:09 2025
...
data \DIFaddbegin \DIFadd{and illustrate its performance in small samples with simulated
data}\DIFaddend .
\end{abstract}
...
\begin{equation*}
  ...
  \left(
      F(t,l,X) - \1{\{\eta(t)=l\}}
  \right)^2.
\end{equation*} The Brier score is \DIFdelbegin \DIFdel{here the }\DIFdelend \DIFaddbegin \DIFadd{the average }\DIFaddend squared prediction
error across \DIFdelbegin \DIFdel{all of }\DIFdelend the four states. Based on a split of a data set
\(\data_n\) into $K$ disjoint approximately equally sized subsets
(c.f., Section \ref{sec:super-learning}), each learner \( \phi_{a_1,
a_2, b} \) in the library \( \mathcal{F}(\mathcal{A}_1, \mathcal{A}_2,
\mathcal{B}) \) is evaluated using the cross-validated loss,
\begin{equation*}
...
%DIF >  ANDERS: to me Poisson regression is a way to fit a Cox model
...
near-parametric rate of convergence. %DIF >  ANDERS: what does "taken equal to mean"? 
The constant \( C_P \) can be
...
as Cox lasso or random survival forests.%DIF > ANDERS: need references here
...
learner achieves the same convergence rate as the best-performing %DIF >  ANDERS: best performing with respect to our integrated Brier score on F?
...
\DIFdelbegin \DIFdel{consider estimation of }\DIFdelend \DIFaddbegin \DIFadd{estimate }\DIFaddend the conditional survival function. \DIFdelbegin \DIFdel{We restrict
attention to the simpler setting without competing risks to enable }\DIFdelend \DIFaddbegin \DIFadd{In order to enable a
}\DIFaddend comparison with existing super learner methods \DIFaddbegin \DIFadd{we restrict the
attention to a setting without competing risks. We report the results
of the numerical experiments in two parts}\DIFaddend . In the first part, we
compare the joint survival super learner \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend two IPCW based discrete
...
depends on the baseline covariates. We simulate data based on }\DIFdelend \DIFaddbegin \DIFadd{For our numerical experiments we have synthesized the data of }\DIFaddend the
prostate cancer study of \cite{kattan2000pretreatment} \DIFaddbegin \DIFadd{based on
structural equations where we specify Cox-Weibull regression models
\mbox{%DIFAUXCMD
\cite{Bender2005} }\hspace{0pt}%DIFAUXCMD
for the latent event and censoring times}\DIFaddend . The
outcome of interest is the time to tumour recurrence, and five
baseline covariates are used to predict outcome: prostate-specific
antigen (PSA, ng/mL), Gleason score sum (GSS, values between 6 and
10), radiation dose (RD), hormone therapy (HT, yes/no) and clinical
stage (CS, six values). The study was designed such that a patient's
radiation dose depended on when the patient entered the study
\citep{gerds2013estimating}. This in turn implies that the time of
...
Cox-Weibull models estimated from the original data, and the
covariates are generated according to either marginal Gaussian normal
or binomial distributions estimated from the original data
